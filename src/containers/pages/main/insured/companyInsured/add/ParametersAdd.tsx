...
@hot(module)
class ParameterAdd extends RootComponent<FormProps, FormState> {
  timerId: any = null

  constructor (props: FormProps) {
    super(props)
    this.state = {
      companyParams: SysUtil.getLocalStorage('CompanyParamsAdd') || {},
      disabled: true,
      projectList: [],
      entityList: [],
      cityList: [],
  ...
  getInitialList () {
    const { projectId = -1 } = this.state.companyParams
    const { companyInsuredProjectList, companyInsuredEntityList, queryCityList, queryInsuredList } = this.api
  ...
  getFieldsValue = (t: number) => {
    ...
    this.timerId = setTimeout(() => {
      ...
      const {
        projectId, entityId, insuredCityId, insuranceStandardsId, startRuleTime, endRuleTime, takeEffectTime
      } = fieldsValue
      const disabled = !(
        projectId && entityId && insuredCityId && insuranceStandardsId && startRuleTime
      )
      this.setState({
        disabled
      })
  ...
  handleSubmit = () => {
    ...
    // const takeEffectTime = moment(params.takeEffectTime).format('YYYY-MM-DD')
    // const [y, m, d] = takeEffectTime.split('-')
    // if (Number(d) > 1) {
    //   this.$message.warn('生效日期仅能选择每月1日', 2)
    //   return
    // }
    // params.takeEffectTime = takeEffectTime
    params.endRuleTime = params.startRuleTime
    this.axios.request(this.api.companyMaintainAdd, params)
      .then(() => {
        this.$message.success('新增成功!本月1号开始生效。')
        SysUtil.clearLocalStorage('CompanyParamsAdd')
  ...
  render () {
    ...
    const { projectId, entityId, insuredCityId, insuranceStandardsId, startRuleTime, endRuleTime, takeEffectTime } = companyParams
    return (
      <div id="company_add_container">
        <Form layout="inline" className="entry-add-from " onSubmit={this.handleSubmit}>
          <Row className="entry-add-label-title">
            <Col><span style={{ fontWeight: 700 }}>新增公司参保维护</span></Col>
          </Row>
          <div>
            <Row className="row_item">
              ...
              <Col span={6}>
                <Form.Item label="法人主体" className="form-item custom-select-width">
        ...
        <Row>
          <Button type="primary" className="ant_button_confirm" disabled={disabled} onClick={this.handleSubmit}>确定</Button>
          <Button className="ant_button_cancel" onClick={this.cancelAdd}>取消</Button>
        </Row>
        <Prompt when message="CompanyParamsAdd" />
      </div>
    )
  }
...
export default Form.create<FormProps>({})(ParameterAdd)
