...
interface CityPageProps extends BaseProps {
  type?: any
}
...
@hot(module)
export default class CityPage extends RootComponent<CityPageProps, CityPageState> {
  ...
  private apiObj:any = { // 下载导出的api
    importApi: this.api.cityImport,
    exportApi: this.api.cityExport,
    exportTemplte: this.api.cityExportTemplate,
    fileName: 'HR参保城市信息导出',
    fileNameTemplte: 'HR参保城市信息导入模板'
  }
  ...
  constructor (props:any) {
    super(props)
    const searchParams = SysUtil.getSessionStorage('CityPage_searchParams')
    this.params = searchParams
    if (searchParams) {
      const { startTime, endTime, standardName } = searchParams
      searchParams.createTimeStart = startTime
      searchParams.createTimeEnd = endTime
      searchParams.icName = standardName
      delete searchParams.startTime
      delete searchParams.endTime
      delete searchParams.standardName
    }
    this.state = {
  ...
  loadingTableData = () => {
    ...
    axios.request(api.cityQuery, obj).then((res:any) => {
  ...
  removeData = () => {
    ...
    axios.request(api.cityRemove, {
      icIdArr: this.state.keys
    }).then((res:any) => {
      if (res.code === 200) {
  ...
  removeFun = (num: number = 0, id?:number, isuse?:number) => {
    if (num === 0) {
      if (isuse) {
        ...
        this.handleModal(1, 'two', `该城市参保参数正被引用/曾被应用，不可删除。`)
      } else {
      ...
      if (this.state.keys.length > 0) {
        if (this.keysName.length > 0) {
          ...
          this.handleModal(1, 'two', `以下城市参保参数正被引用/曾被应用，不可删除`)
        } else {
          this.handleModal(1, 'one', `确认删除?`)
        }
      } else {
  ...
  render () {
    ...
    let paramCity = {
      ...
      standardName: searchParams.icName
    }
    return (
      <div>
        <SearchItem namePlaceholder='请输入城市参保名称' searchParams={paramCity} getSearchData={this.getSearchData} type={2} />
        <BtnColumn apiObj={this.apiObj} searchParams={paramCity}
          addPath = {'/home/parameters/city'} type='city'
          removeFun={this.removeFun} {...this.props}></BtnColumn>
        {dataSource.length > 0 ? <div>
          {dataSource.map((el:any, index:number) => (
            <ColumnsContent type="city" detailFun={this.detailFun} keys={keys} checkChange={this.checkChange}
