...
import './InitialSignatureDetailPage.styl'
...
interface InitialSignatureDetailPageProps extends FormComponentProps {
  // Contract: any
  location?:any
  history?:any
}
@hot(module) // 热更新（局部刷新界面）
class InitialSignatureDetailPage extends RootComponent<InitialSignatureDetailPageProps, any> {
  ...
  componentDidMount () {
    let data = SysUtil.getSessionStorage('InitiaDataDetail')
    this.setState({
      loading: true,
      userId: data.userId,
      id: data.id
  ...
  // 转换图片和 pdf 文件格式，用于上传组件, contractPdf: 短路径, contractPdfUrl: 长路径
  ...
  handleSumbit = async () => {
    ...
    this.axios.request(this.api.contractDetial, dataParm).then((res:any) => {
      if (res.code === 200) {
        ...
        SysUtil.clearSession('InitiaDataDetail')
      }
    }).catch((err: any) => {
      this.error(err.msg[0])
      this.setState({
  ...
  render () {
    ...
    const { userInfo, typeName, hourType, taxationType, probationName, startTime, endTime, images, contractPdf, contractOss } = listArr
    return (
      <div id="contract-detail-page">
        <Spin tip="Loading..." spinning={loading}>
          <Form layout="inline">
            ...
            {
              this.isAuthenticated(this.AuthorityList.contract[3])
...
const ContractForm = Form.create<InitialSignatureDetailPageProps>()(InitialSignatureDetailPage)
