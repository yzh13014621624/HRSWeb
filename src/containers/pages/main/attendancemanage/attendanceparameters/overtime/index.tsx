/*
 * @description: 考勤管理-加班参数参数主页面
 * @author: zhousong
 * @lastEditors: zhousong
 * @Date: 2019-09-23 10:29:06
 * @LastEditTime: 2020-06-08 16:51:55
...
import API from '@server/moudle/basics/salary'
...
const REQUEST_TYPE = 3 // 增删改查接口type值，班次-1，缺勤-2，加班-3

interface OverTimeProps extends FormProps {
}
...
@hot(module)
class OverTime extends RootComponent<OverTimeProps, OverTimeState> {
    ...
    this.props.form.validateFields((err, value: any) => {
      ...
      this.axios.request(this.api.insertScheduling, params, false).then(({ code }) => {
        if (code === 200) {
          ...
          this.removeCachedStorage('overTime')
          this.onCloseModal()
          this.getPageData()
        }
      })
  ...
  onViewDeital = (item: any) => {
    if (!item) {
      this.setState({ modalVisible: true, isEdit: false }, () => {
        const storage = this.getCachedFromStorage('overTime')
  ...
  justifyConfirm = () => {
    ...
    this.timer = setTimeout(() => {
      ...
      if (!isEdit) {
        this.setCachedToStorage('overTime', formValues)
      }
      this.setState({
        disabled
      })
            ...
            <React.Fragment>
              {
                item.paramStatus === 1
                  ...
                  : (
                    <React.Fragment>
                      {isAuthenticated(AuthorityList.composeParam[16]) && <a onClick={() => this.onViewDeital(item)}>查看</a>}
                      {isAuthenticated(AuthorityList.composeParam[17]) && <a className='action-link' onClick={() => this.onOpenDeleteModal(item)}>删除</a>}
                      {isAuthenticated(AuthorityList.composeParam[18]) &&
                        <>
    ...
    return (
      <div className='over-time'>
        <CommonPage
          ...
          loading={loading}
          count={totalCount}
          isAllowAdd={isAuthenticated(AuthorityList.composeParam[14])}
        />
        <Modal
          ...
          destroyOnClose={true}
        >
          <Form.Item label='加班代码' {...formItemLayout}>
            {getFieldDecorator('schedulingCode', {
              ...
              <Select className='modal-form-width' allowClear disabled={isEdit} onChange={this.justifyConfirm} placeholder='请选择'>
                {
                  CodeList.lowerCase().map((item: any, index: number) => {
                    return (
                      <Option key={index} value={`J${item}`}>J{item}</Option>
                    )
                  })
                }
              </Select>
          ...
          <Form.Item label='加班名称' {...formItemLayout}>
          ...
          <div className='modal-button-group'>
            {
              isEdit
                ? isAuthenticated(AuthorityList.composeParam[15]) && <Button className='modal-button' type='primary' disabled={disabled} onClick={this.onSaveEdit}>保存</Button>
                : <Button className='modal-button' type='primary' disabled={disabled} onClick={this.onSaveAdd}>确定</Button>
            }
            {
              isAuthenticated(AuthorityList.composeParam[15]) &&
...
export default Form.create<FormProps>()(OverTime)
