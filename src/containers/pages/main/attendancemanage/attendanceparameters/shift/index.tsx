/*
 * @description: 考勤管理-班次参数主页面
 * @author: zhousong
 * @lastEditors: zhousong
 * @Date: 2019-09-23 10:27:12
 * @LastEditTime: 2020-06-08 16:52:08
...
const { Option } = Select
const REQUEST_TYPE = 1 // 增删改查接口type值，班次-1，缺勤-2，加班-3
...
@hot(module)
class Shift extends RootComponent<ShiftProps, ShiftState> {
  ...
  onViewDeital = (item: any) => {
    if (!item) {
      this.setState({ modalVisible: true, isEdit: false }, () => {
        const storage = this.getCachedFromStorage('shift')
        ...
        // this.setState({
        //   dailyAttendance: storage && storage.dailyAttendance === 2
        // })
        let hasEdit = false
      ...
      this.setState({
        ...
        this.props.form.setFieldsValue({
          ...
          'marks': item.marks,
          // 'dailyAttendance': item.dailyAttendance || 1,
    ...
    this.props.form.validateFields((err, value: any) => {
      ...
      this.axios.request(this.api.insertScheduling, params, false).then(({ code }) => {
        if (code === 200) {
          ...
          this.removeCachedStorage('shift')
          this.onCloseModal()
          this.getPageData()
        }
      })
  ...
  justifyConfirm = () => {
    ...
    this.timer = setTimeout(() => {
      ...
      _.forEach(formValues, (value, key) => {
        ...
        if ((value === undefined || value === null) && key !== 'marks') {
          disabled = true
        }
        // } else {
        //   if (key !== 'startWorkTime' && key !== 'endWorkTime' && key !== 'breakTime' && key !== 'marks') {
      ...
      if (!isEdit) {
        this.setCachedToStorage('shift', formValues)
      }
      this.setState({
        disabled
      })
  ...
  render () {
    const {
      isAuthenticated,
      AuthorityList,
      state: { modalVisible, tableData, loading, isEdit, warnMsg, errorBtn, totalCount, disabled, dailyAttendance, disableName, startOpen, endOpen },
      props: {
        ...
        render: (value: any, item: any, index: any) => {
          return (
            <React.Fragment>
              {
                item.paramStatus === 1
                  ? <React.Fragment>{(isAuthenticated(AuthorityList.composeParam[5]) || isAuthenticated(AuthorityList.composeParam[6])) && <a onClick={() => this.onViewDeital(item)}>查看</a>}</React.Fragment>
                  : (
                    <React.Fragment>
                      {isAuthenticated(AuthorityList.composeParam[6]) && <a onClick={() => this.onViewDeital(item)}>查看</a>}
                      {isAuthenticated(AuthorityList.composeParam[7]) && <a className='action-link' onClick={() => this.onOpenDeleteModal(item)}>删除</a>}
                      {isAuthenticated(AuthorityList.composeParam[8]) &&
                        <>
    ...
    return (
      <div className='shift'>
        <CommonPage
          ...
          loading={loading}
          count={totalCount}
          isAllowAdd={isAuthenticated(AuthorityList.composeParam[4])}
        />
        <Modal
          ...
          destroyOnClose={true}
        >
          <Form.Item label='班次代码' {...formItemLayout}>
            {getFieldDecorator('schedulingCode', {
              ...
              <Select placeholder='请选择' allowClear disabled={isEdit} onChange={this.justifyConfirm} className='modal-form-width'>
                {
                  CodeList.uperCase().map((item: any, index: number) => {
                    return (
          ...
          <Form.Item label='班次名称' {...formItemLayout}>
            {getFieldDecorator('schedulingName', {
              ...
              <Input maxLength={20} allowClear disabled={disableName} placeholder='请输入' onChange={this.justifyConfirm} className='modal-form-width'/>
            )}
          </Form.Item>
          ...
          <Form.Item label='备注' {...formItemLayout}>
            {getFieldDecorator('marks', {})(
              <Input.TextArea maxLength={50} placeholder='最多可输入50字' onChange={this.justifyConfirm} className='modal-form-width' />
            )}
          </Form.Item>
          <div className='modal-button-group'>
            {
              isEdit
                ? isAuthenticated(AuthorityList.composeParam[5]) && <Button className='modal-button' type='primary' disabled={disabled} onClick={this.onSaveEdit}>保存</Button>
                : <Button className='modal-button' type='primary' disabled={disabled} onClick={this.onSaveAdd}>确定</Button>
            }
            {
              isAuthenticated(AuthorityList.composeParam[5]) &&
...
export default Form.create<ShiftProps>()(Shift)
